...
#     radar1.draw('./static/images/diagram.png', initial_equations, data[0], u_list,
#                 "Характеристики системы: начальный момент времени", show_both_lines=False)
    
#     # Остальные графики - обе линии
#     radar1.draw('./static/images/diagram2.png', initial_equations, data[int(len(data) / 4)], u_list,
#                 "Характеристики системы: 1 четверть времени", show_both_lines=True)
#     radar1.draw('./static/images/diagram3.png', initial_equations, data[int(len(data) / 2)], u_list,
#                 "Характеристики системы: 2 четверть времени", show_both_lines=True)
#     radar1.draw('./static/images/diagram4.png', initial_equations, data[int(len(data) / 4 * 3)], u_list,
#                 "Характеристики системы: 3 четверть времени", show_both_lines=True)
#     radar1.draw('./static/images/diagram5.png', initial_equations, data[-1, :], u_list,
...
#     """Создает график внешних воздействий"""
...
#         disturbance_values = f3(t, faks[i])
 
#         axs.plot(t, disturbance_values, color=colors[i], linewidth=2.5, label=disturbances_labels[i])
...
def create_disturbances_graphic(t, faks):
    ...
    # Внешние воздействия с правильными названиями
    disturbances_labels = [
        "S(t) - Площадь зоны затопления",
        "F(t) - Скорость течения воды", 
        "G(t) - Глубина воды",
    ...
    for i in range(7):
        # Вычисляем значения возмущений во времени
        disturbance_values = f3(t, faks[i])
        # Обрезаем значения для внешних воздействий тоже
        disturbance_values = np.clip(disturbance_values, 0, 1.0)
        axs.plot(t, disturbance_values, color=colors[i], linewidth=2.5, label=disturbances_labels[i])
    
    axs.set_xlim([0, 1])
    axs.set_ylim([0, 1])
    axs.set_xlabel("t, время", fontsize=14, fontweight='bold')
    axs.set_ylabel("Значения воздействий", fontsize=14, fontweight='bold')  # Убрал "Нормализованные"
    axs.set_title("График внешних воздействий на систему", fontsize=16, fontweight='bold', pad=20)
    axs.legend(loc='upper right', fontsize=10, framealpha=0.9)
    axs.grid(True, alpha=0.3, linestyle='--')
    axs.tick_params(axis='both', which='major', labelsize=12)
