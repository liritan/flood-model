...
with tab4:
    ...
    if st.session_state.calculation_done and st.session_state.free_members is not None:
        ...
        st.subheader("Информация о функциях")
        st.write("""
        На графиках показаны все 6 функций системы с вашими коэффициентами:
        - **F1**: Кубический полином - ax³ + bx² + cx + d
        - **F2, F4, F6**: Линейные функции - ax + b  
        - **F3, F5**: Квадратные полиномы - ax² + bx + c
        """)
        ...
        st.download_button(
            label="📥 Скачать все графики функций",
            data=buf.getvalue(),
            file_name="все_графики_функций.png",
            mime="image/png",
            use_container_width=True
        )
        
    else:
        st.info("Выполните вычисления на вкладке 'Параметры' чтобы увидеть графики функций")
